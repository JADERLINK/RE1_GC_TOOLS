...
{
    public static class Re1GcBinDecoder
    {
        public static RE1GCBIN Decoder(Stream stream, long startOffset, out long endOffset) 
        {
            RE1GCBIN BIN = new RE1GCBIN();
            ...
            (int Vertex_Count, int Normal_Count, int UV_Count) True_Count;

            //material
            BIN.Materials = Materials(br, BIN.Header.material_offset + startOffset, BIN.Header.material_count, out True_Count);
            endOffset = br.BaseStream.Position;

            //--------

            BIN.Vertex_Position_Array = Get_Vertex_Position_Array(br, BIN.Header.vertex_position_offset + startOffset, True_Count.Vertex_Count);
        ...
        private static MaterialBin[] Materials(BinaryReader br, long offset, ushort MatCount, out (int Vertex_Count, int Normal_Count, int UV_Count) True_Count) 
        {
            ...
            {
                materials[i] = Get_Material(br, ref index_Vertex, ref index_Normal, ref index_UV);
            }

            True_Count = (index_Vertex + 1, index_Normal + 1, index_UV + 1);
            return materials;
        }

        private static MaterialBin Get_Material(BinaryReader br, ref int index_Vertex, ref int index_Normal, ref int index_UV) 
        {
            ...
            group.face_index_array = Get_face_index(br, ref index_Vertex, ref index_Normal, ref index_UV);

            return group;
        }

        private static (FaceIndex i1, FaceIndex i2, FaceIndex i3)[] Get_face_index(BinaryReader br, ref int index_Vertex, ref int index_Normal, ref int index_UV) 
        {
            ...
            while (true)
            {
                if (br.BaseStream.Position >= tempOffset + buffer_size || br.BaseStream.Position >= br.BaseStream.Length) // evita loop infinito, caso houver um erro
                {
                    break;
                }
                ...
                if (ftype == 0x98 || ftype == 0x90 || ftype == 0x80 || ftype == 0xA0)
                {
                    ...
                    Get_face_index_parts(br, ref face_index_list, fcount, ftype, ref index_Vertex, ref index_Normal, ref index_UV);
                }
                else 
                {
                    break;
        ...
        private static void Get_face_index_parts(BinaryReader br, ref List<(FaceIndex i1, FaceIndex i2, FaceIndex i3)> face_index_list,
            ushort fcount, byte ftype, ref int index_Vertex, ref int index_Normal, ref int index_UV)
        {
        ...
        private static (short nx, short ny, short nz, byte color_index, byte weightmap_index)[] Get_Vertex_Normal_Array(BinaryReader br, long offset, int count) 
        {
            (short nx, short ny, short nz, byte color_index, byte weightmap_index)[] normals = 
                new (short nx, short ny, short nz, byte color_index, byte weightmap_index)[count];
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                byte color_index = br.ReadByte();
                byte weightmap_index = br.ReadByte();
                normals[i] = (nx, ny, nz, color_index, weightmap_index);

            }
            return normals;
        }

        private static (short vx, short vy, short vz, byte color_index, byte weightmap_index)[] Get_Vertex_Position_Array(BinaryReader br, long offset, int count) 
        {
            (short vx, short vy, short vz, byte color_index, byte weightmap_index)[] positions = 
                new (short vx, short vy, short vz, byte color_index, byte weightmap_index)[count];
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                byte color_index = br.ReadByte();
                byte weightmap_index = br.ReadByte();
                positions[i] = (vx, vy, vz, color_index, weightmap_index);
            }
            return positions;
        }

        private static Re1GcBinHeader GetHeader(BinaryReader br) 
        {
            Re1GcBinHeader header = new Re1GcBinHeader();

            header.bone_offset = br.ReadUInt32();

            if (header.bone_offset != 0x00000040)
            {
